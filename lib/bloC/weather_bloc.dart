    ...
    on<getWeather>((event, emit) async {
      ...
      if (isOnline == true) {
        var response = await dio.get(
            "http://api.weatherapi.com/v1/forecast.json?key=8c9416d38c45437a8a9105439231109&q=${event.city??'Tashkent'}&days=4");
        box.put("weather", WeatherApi.fromJson(response.data));
        emit(NetworkSuccess(WeatherApi.fromJson(response.data)));
      } else {
        emit(NetworkSuccess(box.get("weather") as WeatherApi));
      }
